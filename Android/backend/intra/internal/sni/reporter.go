...
var _ TCPSNIReporter = (*tcpSNIReporter)(nil)

func MakeTCPReporter(dns doh.Transport) TCPSNIReporter {
	return &tcpSNIReporter{
		dns: dns,
	}
}
...
func (r *tcpSNIReporter) Send(report choir.Report) error {
	...
	if err != nil {
		log.Printf("[warn] Failed to construct query for Choir: %v\n", err)
		return nil
	}
	if _, err = dns.Query(q); err != nil {
		log.Printf("[info] Failed to deliver query for Choir: %v\n", err)
	}
	return nil
}
...
func (r *tcpSNIReporter) Report(summary TCPSummary) {
