...
public class LocalhostResolverTest {
  ...
  @Captor ArgumentCaptor<DnsTransaction> transactionCaptor;
  ...
  public void success() throws Exception {
    ...
    DnsTransaction transaction = transactionCaptor.getValue();
    ...
    assertEquals(DnsTransaction.Status.COMPLETE, transaction.status);
  ...
  public void error404() throws Exception {
    ...
    assertEquals(QUERY.timestamp, transaction.queryTime);
    assertEquals(SERVER_IP, transaction.serverIp);
    assertEquals(DnsTransaction.Status.HTTP_ERROR, transaction.status);
  ...
  public void sendFailed() throws Exception {
    ...
    assertEquals(DnsTransaction.Status.SEND_FAIL, transaction.status);
    assertNull(transaction.response);
  }
  ...
  public void canceled() throws Exception {
    ...
    assertEquals(query.type, transaction.type);
    assertNull(transaction.serverIp);
    assertEquals(DnsTransaction.Status.CANCELED, transaction.status);
    assertNull(transaction.response);
  }
