    ...
    try {
      ...
      if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
        ...
        try {
          ...
          for (String packageName : PersistentState.getExcludedPackages(vpnService)) {
  ...
  public void run() {
    ...
    while (!isInterrupted()) {
      try {
        ...
        DnsResolverUdpToHttps.processQuery(vpnService.getServerConnection(), dnsRequest, udpPacket.data, this);
      } catch (Exception e) {
        if (!isInterrupted()) {
